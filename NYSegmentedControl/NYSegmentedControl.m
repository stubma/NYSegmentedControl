...
- (CGRect)indicatorFrameForSegment:(NYSegment *)segment {
	if(self.delegate && [(id)self.delegate respondsToSelector:@selector(segmentedControl:indicatorFrameForSegment:)]) {
		return [self.delegate segmentedControl:self indicatorFrameForSegment:segment];
	} else {
